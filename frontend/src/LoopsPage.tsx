  ...
  return (
    <section className="page loops-page">
      ...
      <div className="loop-kind-grid">
        <article className="loop-kind active"><span><Target size={24} /></span><div><strong>Goal-based Loop</strong><p>新建或接入现有 Session，自主迭代直至完成。</p><small>可选 /goal · AI 自动审批与选择 · 无硬性轮次上限</small></div><em>已开放</em></article>
          ...
          {loading && !loops.length ? <div className="loop-loading"><LoaderCircle className="spin" />正在读取 Goal</div> : (
            <div className="loop-table">
              ...
              {visibleLoops.map((loop) => <div key={loop.id} role="button" tabIndex={0} className={`loop-row interactive ${selected?.id === loop.id ? "selected" : ""}`} onClick={() => setSelectedID(loop.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedID(loop.id); } }}>
                ...
                <LoopRowActions loop={loop} busy={!!busy} onEdit={() => openEditor(loop)} onStart={() => { if (loop.useGoalCommand && !window.confirm("请确认 OpenCode Agent 已支持 /goal。确认后立即启动？")) return; void apply("start", () => AppService.StartGoalLoop(loop.id, loop.useGoalCommand)); }} onRestart={() => { if (!window.confirm(`将复用当前配置和原 Session，重新发送${loop.useGoalCommand ? " /goal" : "目标"}；若 Session 正忙会先等待。确定重新启动？`)) return; void apply("restart", () => AppService.RestartGoalLoop(loop.id, loop.useGoalCommand)); }} onPause={() => void apply("pause", () => AppService.PauseGoalLoop(loop.id))} onResume={() => void apply("resume", () => AppService.ResumeGoalLoop(loop.id))} onConfirm={() => void apply("confirm", () => AppService.ConfirmGoalLoopComplete(loop.id))} onApprovals={() => openLoopApprovals(loop)} onOpenSession={() => void openSession(loop)} onTerminate={() => terminateLoop(loop)} onTerminateSession={() => { if (window.confirm("这会同时中断 OpenCode Session 当前执行，确定继续？")) void apply("terminate-session", () => AppService.TerminateGoalLoopAndSession(loop.id)); }} onDelete={() => deleteLoop(loop)} />
              </div>)}
...
function GoalEditor({ current, projects, sessions, initialSession, models, modelsLoading, modelsError, reloadModels, loops, onClose, onSaved, showToast }: { current: GoalLoopView | null; projects: ProjectView[]; sessions: SessionView[]; initialSession: SessionView | null; models: GoalModelView[]; modelsLoading: boolean; modelsError: string; reloadModels: () => Promise<void>; loops: GoalLoopView[]; onClose: () => void; onSaved: (page: GoalLoopPage) => void; showToast: (message: string) => void }) {
  ...
  const save = async (startNow: boolean) => {
    ...
    if (startNow && useGoalCommand && !commandConfirmed) { showToast("请确认 Agent 支持 /goal"); return; }
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="goal-modal" role="dialog" aria-modal="true" aria-label={current ? "编辑 Goal" : "创建 Goal"}>
    ...
    <div className="goal-modal-body">
      ...
      <div className="loop-form-grid"><label className="loop-form-field"><span>Agent</span><select value="opencode-default" disabled><option value="opencode-default">OpenCode</option><option disabled>Claude Code（即将支持）</option><option disabled>Codex（即将支持）</option></select><small>{useGoalCommand ? "Goal 启动消息会添加 /goal 前缀" : "Goal 启动消息直接发送目标文本"}</small></label><label className="loop-form-field"><span>项目</span><select value={projectID} disabled={terminalEdit} onChange={(event) => { setProjectID(event.target.value); setSessionID(""); }}><option value="">选择已接入飞书的项目</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name} · {project.directory}</option>)}</select><small>{terminalEdit ? "终态编辑保留原项目" : "多个 Loop 同项目运行时推荐使用独立 worktree"}</small></label></div>
