...
export default function LoopsPage({ projects, sessions, initialSession, onInitialSessionConsumed, showToast }: Props) {
  ...
  return (
    <section className="page loops-page">
        ...
        <section className="panel loop-detail-panel">
          {selected ? <>
            ...
            <div className="loop-detail-body">
              ...
              <div className="loop-detail-grid"><Detail label="自主策略"><strong>{selected.automationMode === "manual" ? "人工监督" : "完全自主"}</strong><small>{selected.automationMode === "manual" ? "应用或飞书处理请求" : "AI 自动审批并回答选择框"}</small></Detail><Detail label="监督模型"><strong>{selected.supervisorModelName || selected.supervisorModelId || selected.modelName}</strong><small>{selected.supervisorModelId === "__agent_default__" ? "Agent 默认模型" : `${selected.supervisorModelProviderId}/${selected.supervisorModelId}`}</small></Detail></div>
      ...
      {editor !== undefined && <GoalEditor current={editor} projects={connectedProjects} sessions={sessions} initialSession={attachSession} models={models} modelsLoading={modelsLoading} modelsError={modelsError} reloadModels={loadModels} loops={loops} onClose={() => { setEditor(undefined); setAttachSession(null); }} onSaved={(page) => { setData({ ...page, loops: page.loops ?? [], approvals: page.approvals ?? [] }); setEditor(undefined); setAttachSession(null); showToast(editor ? "草稿已更新" : "Goal 已创建"); }} showToast={showToast} />}
...
function GoalEditor({ current, projects, sessions, initialSession, models, modelsLoading, modelsError, reloadModels, loops, onClose, onSaved, showToast }: { current: GoalLoopView | null; projects: ProjectView[]; sessions: SessionView[]; initialSession: SessionView | null; models: GoalModelView[]; modelsLoading: boolean; modelsError: string; reloadModels: () => Promise<void>; loops: GoalLoopView[]; onClose: () => void; onSaved: (page: GoalLoopPage) => void; showToast: (message: string) => void }) {
  ...
  const save = async (startNow: boolean) => {
    ...
    const input: GoalLoopInput = {
      ...
      sessionId: source === "existing" ? sessionID : "", automationMode,
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="goal-modal" role="dialog" aria-modal="true" aria-label={current ? "编辑 Goal" : "创建 Goal"}>
    ...
    <div className="goal-modal-body">
      ...
      <div className="loop-source-tabs"><button type="button" className={source === "new" ? "active" : ""} onClick={() => { setSource("new"); setSessionID(""); }}>新建 Session</button><button type="button" className={source === "existing" ? "active" : ""} onClick={() => setSource("existing")}>接入现有 Session</button></div>
      <div className="loop-form-grid"><label className="loop-form-field"><span>Agent</span><select value="opencode-default" disabled><option value="opencode-default">OpenCode</option><option disabled>Claude Code（即将支持）</option><option disabled>Codex（即将支持）</option></select><small>Goal 接管消息自动添加 /goal 前缀</small></label><label className="loop-form-field"><span>项目</span><select value={projectID} onChange={(event) => { setProjectID(event.target.value); setSessionID(""); }}><option value="">选择已接入飞书的项目</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name} · {project.directory}</option>)}</select><small>多个 Loop 同项目运行时推荐使用独立 worktree</small></label></div>
      {source === "existing" && <label className="loop-form-field"><span>现有 Session</span><select value={sessionID} onChange={(event) => setSessionID(event.target.value)}><option value="">选择该项目的顶层 Session</option>{eligibleSessions.map((session) => <option value={session.id} key={session.id}>{session.title} · {shortID(session.id)} · {session.statusLabel}</option>)}</select><small>{selectedSession?.currentModel ? `默认继承原模型 ${selectedSession.currentModel}${selectedSession.currentVariant ? ` · ${selectedSession.currentVariant}` : ""}；你也可以修改，新模型从 /goal 下一轮生效。` : "当前执行不会被打断；现有权限和选择请求会立即由 Goal 接管。"}</small></label>}
      ...
      <div className="loop-mode-grid"><button type="button" className={automationMode === "autonomous" ? "active" : ""} onClick={() => setAutomationMode("autonomous")}><Sparkles size={17} /><span><strong>完全自主（推荐）</strong><small>AI 自动审批权限、回答选择框并寻找安全替代方案</small></span></button><button type="button" className={automationMode === "manual" ? "active" : ""} onClick={() => setAutomationMode("manual")}><ShieldCheck size={17} /><span><strong>人工监督</strong><small>在应用或飞书中处理权限和问题</small></span></button></div>
      {automationMode === "autonomous" && <div className="loop-model-config"><div className="loop-model-config-title"><span>AI 监督高级设置</span><small>独立 Session，只输出结构化决策</small></div><div className="loop-form-grid"><label className="loop-form-field"><span>监督模型</span><select value={supervisorKey} onChange={(event) => { setSupervisorKey(event.target.value); setSupervisorVariant(""); }}><option value="">跟随 Goal 执行模型</option>{models.map((model) => <option value={`${model.providerId}\u0000${model.id}`} key={`supervisor-${model.providerId}/${model.id}`}>{model.providerName || model.providerId} · {model.name || model.id}</option>)}</select><small>可选更快模型；失败后自动回退到执行模型</small></label><label className="loop-form-field"><span>监督模型档位</span><select value={supervisorVariant} onChange={(event) => setSupervisorVariant(event.target.value)} disabled={!supervisorModel || !(supervisorModel.variants ?? []).length}><option value="">默认档位</option>{(supervisorModel?.variants ?? []).map((variant) => <option value={variant} key={variant}>{variant}</option>)}</select><small>仅影响权限和选择决策</small></label></div><label className="loop-form-field"><span>额外允许目录（每行一个）</span><textarea className="compact" value={allowedDirectories} onChange={(event) => setAllowedDirectories(event.target.value)} placeholder="例如 D:\shared\assets" /><small>项目目录默认允许；系统破坏、凭据和未知数据外传仍会被硬性拦截</small></label></div>}
