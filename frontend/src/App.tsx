...
function SessionDetailDialog({ session, onClose, onRefresh, showToast }: { session: SessionView; onClose: () => void; onRefresh: () => void; showToast: (message: string) => void }) {
  ...
  const [currentOperationExpanded, setCurrentOperationExpanded] = useState(false);
  const noActivity = useLiveSeconds(session.noActivitySeconds, isSessionBusy(session) && session.activityLevel !== "waiting");
  ...
  const currentOperationInput = currentOperation?.inputPreview ?? "";
  useEffect(() => { setCurrentOperationExpanded(false); }, [currentOperation?.id]);
  ...
  const copyCurrentOperationInput = async () => {
    try {
      await navigator.clipboard.writeText(currentOperationInput);
      showToast("已复制当前操作输入");
    } catch (reason) {
      showToast(`复制失败：${errorMessage(reason)}`);
    }
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="session-detail-modal" role="dialog" aria-modal="true" aria-label="Session 详情">
    ...
    <div className="session-detail-body">
      <div className={`stall-banner ${session.activityLevel}`}><CircleAlert size={19} /><div><strong>{activityStatusLabel(session)}</strong><span>{isSessionBusy(session) ? `已经 ${formatDuration(noActivity)} 没有检测到新动作` : `最后活动于 ${formatDateTime(session.lastActivityAt || session.updatedAt)}`}</span>{isSessionBusy(session) && <div className={`current-operation${currentOperationExpanded ? " expanded" : ""}`}><button type="button" className="current-operation-toggle" disabled={!currentOperationInput} aria-expanded={currentOperationExpanded} onClick={() => setCurrentOperationExpanded((expanded) => !expanded)}><span><code>{currentOperation?.tool || session.operationType || "Agent"}</code><b title={currentOperation?.summary || session.operationSummary}>{currentOperation?.summary || session.operationSummary || "正在处理当前任务"}</b></span>{currentOperationInput && (currentOperationExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />)}</button><small>{currentOperation ? `${currentOperation.fromSubagent ? `Subagent · ${currentOperation.sessionTitle}` : "主 Agent"} · 已运行 ${formatDuration(currentOperation.durationSeconds)}` : `${session.activityFromSubagent ? `Subagent · ${session.activitySourceTitle}` : "主 Agent"}${session.operationStatus ? ` · ${session.operationStatus}` : ""}`}{runningOperations.length > 1 ? ` · 另有 ${runningOperations.length - 1} 个操作并行执行` : ""}</small>{currentOperationInput && (currentOperationExpanded ? <div className="current-operation-input-panel"><div><span>输入参数</span><button type="button" title="复制输入" onClick={() => void copyCurrentOperationInput()}><Copy size={14} />复制</button></div><pre>{currentOperationInput}</pre></div> : <code className="current-operation-input" title={currentOperationInput} onClick={() => setCurrentOperationExpanded(true)}>{currentOperationInput}</code>)}</div>}</div></div>
